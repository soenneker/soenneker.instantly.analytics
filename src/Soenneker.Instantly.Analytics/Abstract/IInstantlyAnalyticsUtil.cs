...
public interface IInstantlyAnalyticsUtil
{
    ...
    /// <see cref="GetCampaignAnalytics200ResponseSchemaItem"/> if the campaign exists, or <c>null</c> if no data is found.
    /// </returns>
    [Pure]
    ValueTask<GetCampaignAnalytics200ResponseSchemaItem?> GetCampaignCount(string campaignId, DateTimeOffset startAt, DateTimeOffset? endAt = null, CancellationToken cancellationToken = default);
    ...
    /// <see cref="GetCampaignAnalytics200ResponseSchemaItem"/> instances, or <c>null</c> if no data is found.
    /// </returns>
    [Pure]
    ValueTask<List<GetCampaignAnalytics200ResponseSchemaItem>?> GetCampaignsCounts(DateTimeOffset startAt, DateTimeOffset? endAt = null, CancellationToken cancellationToken = default);
